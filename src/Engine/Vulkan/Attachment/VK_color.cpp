...
//Main function
void VK_color::create_color_attachment(Image* image){
  //---------------------------

  image->color.format = find_color_format();
  vk_texture->create_image(param_vulkan->extent.width, param_vulkan->extent.height, image->color.format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, image->color.image, image->color.mem);
